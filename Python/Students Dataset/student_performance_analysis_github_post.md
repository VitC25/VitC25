...
data = data.rename(columns={"mark": "scores"})
...
## 10. Further Analysis Ideas

The project could be extended by:

- comparing average scores by class;
- comparing score distributions by gender;
- identifying possible outliers;
- creating score bands such as Fail, Pass, Merit, and Distinction;
...
```python
average_by_class = (
    data.groupby("class", dropna=False)["scores"]
        .mean()
...
A histogram could also show how scores are distributed:
